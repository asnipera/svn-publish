...
export function ExecSvnCmd(cmd: string, dir?: string) {
    ...
    const output = spawnSync(fileName, cmd.split(' '), { encoding: 'utf-8', cwd: dir });
    if (output.stderr) {
        console.error('SVN命令执行错误: ', 'svn', cmd);
        if (output.stderr.includes('credentials') || output.stderr.includes('Authentication') || output.stderr.includes('time')) {
...
function update(conf: Config, msg = 'update') {
    createTempRepository(conf.remoteDirectory);
    const repo = repository();
    addFiles(conf);
...
export function Publish(msg: string) {
    ...
    try {
        rmdir(tempRepositoryDir());
        if (conf.compress) fs.unlinkSync(path.join(conf.localDirectory, conf.zipFileName));
    }catch(e) {}
}